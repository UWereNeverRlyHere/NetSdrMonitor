...
namespace NetSdrMonitor.Communication.Client;

/// <summary>
/// Оркестратор лінії NetSDR. Зводить разом транспорт (байти + буфер), протокол (framing і правила
/// відповіді) та парсер (повідомлення-сигнал), віддаючи назовні чистий потік <see cref="Signal"/>.
/// Єдине місце технічних логів цієї лінії.
/// </summary>
public sealed class SignalClient(
    ITransport transport,
    ISdrMessageParser parser,
    ISdrProtocol protocol,
    ILogger<SignalClient> logger) : ISignalClient
{
    ...
    // Результат обробки одного повідомлення: скільки байтів спожито й декодований сигнал (якщо був).
